...
export async function generateMetadata({ searchParams }: { searchParams: Promise<SearchParams> }): Promise<Metadata> {
  ...
  const title = filters.location ? `Sharing Flat in ${filters.location} | Rooms & PGs for Rent (${year})` : "Find Flatmates, Rooms & PGs to Rent Near You";
  const description = filters.location
    ? `Looking for a sharing flat for rent in ${filters.location}? Browse verified, fully furnished single rooms, flats, and budget PGs — filter by budget, property type, and gender preference on FlatFolks.`
