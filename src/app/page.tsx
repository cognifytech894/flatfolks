        ...
        <section className="mx-auto max-w-[1600px] px-5 py-5 sm:px-8 lg:px-12 xl:px-16">
          ...
          <section className="mt-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-7">
              {cities.map(([name, image]) => <div key={name} className="group relative h-[86px] overflow-hidden rounded-lg bg-slate-200 shadow-sm"><Unsplash id={image} alt={name} className="object-cover transition duration-500 group-hover:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/5 to-transparent" /><b className="absolute bottom-2 left-2 right-2 text-center text-[11px] text-white">{name}</b></div>)}
            </div>
          </section>
          <section className="mt-5">
