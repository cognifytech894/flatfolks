...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const body = await request.json() as { name?: string; email?: string; phone?: string; location?: string; gender?: "Boy" | "Girl" };
  if (body.name !== undefined && !body.name.trim()) return NextResponse.json({ error: "Name is required." }, { status: 400 });
  if (body.gender !== undefined && !["Boy", "Girl"].includes(body.gender)) return NextResponse.json({ error: "Invalid gender." }, { status: 400 });
