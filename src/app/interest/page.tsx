...
function InterestForm() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-3xl"><BackLink label="Back to home" /><p className="mt-8 text-sm text-slate-500">Loading listing…</p></div></div>;

  if (listingId && !listing) {
    return <div className="min-h-screen bg-slate-50 px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-3xl"><BackLink label="Back to home" /><div className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm"><h1 className="text-2xl font-semibold text-slate-950">Listing not found</h1><p className="mt-3 text-sm text-slate-600">This listing may have been removed. Go back and try another one.</p></div></div></div>;
  }
