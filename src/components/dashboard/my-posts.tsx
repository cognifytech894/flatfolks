...
import { Camera, Loader2, Pencil, Save, Trash2, X } from "lucide-react";
import { useEffect, useState } from "react";
import { compressImageFile } from "@/lib/compress-image";
import { PreferencesField } from "@/components/listing/preferences-field";

type Post = { id: string; title: string; description?: string; location: string; rent: number; deposit?: number; propertyType: string; listingKind?: "flat-offer" | "flat-requirement"; availableFrom?: string; genderPreference?: "Male" | "Female" | "Family" | "Any"; tags: string[]; images?: string[]; contactPhone?: string; bedrooms?: number; bathrooms?: number; preferences?: string[] };
...
export function MyPosts() {
  ...
  async function save() {
    if (!draft) return;
    if (uploadingPreviews.size > 0) { setMessage("Please wait for your photos to finish uploading."); return; }
    setMessage("");
    const response = await fetch("/api/listings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: draft.id, title: draft.title, description: draft.description, location: draft.location, rent: Number(draft.rent), deposit: Number(draft.deposit) || 0, propertyType: draft.propertyType, bedrooms: draft.bedrooms, bathrooms: draft.bathrooms, availableFrom: draft.availableFrom, genderPreference: draft.genderPreference, tags: draft.tags, images: draft.images || [], contactPhone: draft.contactPhone, preferences: draft.preferences || [] }) });
  ...
  function addDraftImages(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    files.forEach((file) => {
      compressImageFile(file).then(async (compressed) => {
        setDraft((current) => current ? { ...current, images: [...(current.images || []), compressed].slice(0, 3) } : current);
        setUploadingPreviews((current) => new Set(current).add(compressed));
        try {
          ...
          setDraft((current) => current ? { ...current, images: (current.images || []).map((image) => image === compressed ? url : image) } : current);
        } catch (error) {
          setDraft((current) => current ? { ...current, images: (current.images || []).filter((image) => image !== compressed) } : current);
          setMessage(error instanceof Error ? error.message : "Could not upload photo.");
        } finally {
          setUploadingPreviews((current) => { const next = new Set(current); next.delete(compressed); return next; });
        }
      });
    });
  }
