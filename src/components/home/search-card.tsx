...
export function SearchCard() {
  ...
  return <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white/95 p-3 shadow-[0_15px_35px_rgba(15,23,42,.11)] backdrop-blur"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-6">
    ...
    <label className={labelClass}><span className={labelTextClass}><UsersRound className="h-3.5 w-3.5 text-blue-600" />Gender Preference</span><select value={gender} onChange={(event) => setGender(event.target.value)} className={controlClass}><option>Any</option><option>Boy</option><option>Girl</option><option>Family</option></select></label>
