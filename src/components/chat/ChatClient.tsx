...
export default function ChatClient() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      const storedRequests = window.localStorage.getItem("flatfolks_chat_requests");
      if (storedRequests) setRequestedOwners(JSON.parse(storedRequests) as string[]);
    });
    ...
    <main className="min-h-screen bg-slate-50 py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[260px_1fr]">
          <ContactList contacts={requestedOwners} selected={peer} onSelect={(p) => setPeer(p)} />
