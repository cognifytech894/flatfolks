...
type ResultListing = {
  ...
  minBudget: number; maxBudget: number; propertyType: string;
};
        ...
        {sorted.map((listing) => (
          <article key={listing.id} className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-4">
              ...
              <p className="mt-2 text-sm font-medium text-emerald-600">Budget: ₹{listing.minBudget.toLocaleString("en-IN")} - ₹{listing.maxBudget.toLocaleString("en-IN")}</p>
